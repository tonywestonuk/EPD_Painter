...
void setup() {
   ...
   if (display.getConfig().i2c.wire == nullptr) {                                                                                                  
    ...
    for (int i = 0; i < 3; i++) {
        ...
        lv_obj_align(btn, LV_ALIGN_TOP_MID, (i - 1) * 260, 410);
        lv_obj_set_style_bg_color(btn, EPD_PainterLVGL::DK_GREY, 0);
        lv_obj_set_style_bg_color(btn, EPD_PainterLVGL::BLACK, LV_STATE_PRESSED);
        lv_obj_set_style_radius(btn, 8, 0);
        lv_obj_add_event_cb(btn, button_event_cb, LV_EVENT_CLICKED, (void*)(intptr_t)(i + 1));

        lv_obj_t *lbl = lv_label_create(btn);
        lv_label_set_text(lbl, btn_labels[i]);
        lv_obj_set_style_text_color(lbl, EPD_PainterLVGL::WHITE, 0);
